...
- `ClientID`: a big-endian unsigned 32-bit integer representing globally unique identifier of the current client.
  ...
  in [MetaStore](./stores/meta_store.md) and then reused between process sessions. If `MetaStore` doesn't contain
...
called [StateVectorStore](stores/state_vector_store.md).